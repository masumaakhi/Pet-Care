...
const VolunteerRescueHistoryPage = () => {
  const volunteerId = "VOL-001"; // Simulated logged in volunteer

  // Find rescues assigned to this volunteer
  const myHistory = rescueRequests.filter(req => req.assignedVolunteer?.id === volunteerId);

  const completedRescues = myHistory.filter(req => ['rescued', 'shelter'].includes(req.status));

  const stats = {
    completed: completedRescues.length,
    active: myHistory.filter(req => !['rescued', 'shelter'].includes(req.status)).length,
    avgResponse: "12 mins",
    rating: "4.8/5.0"
  };
